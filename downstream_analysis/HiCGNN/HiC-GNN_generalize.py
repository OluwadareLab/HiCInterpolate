...
if __name__ == "__main__":
    ...
    if not(os.path.isfile(f"{out_path}/" + name_trained + '_weights.pt')):
        print('Failed to find model weights corresponding to ' + filepath_trained + ' from ' + f"{out_path}/" + name_trained + '_weights.pt')
        tempmodels = []
        tempspear = []
        tempmse = []
        model_list = []
        ...
        with open(f"{out_path}/" + name_trained + '_log.txt', 'w') as f:
            line1 = f'Optimal conversion factor: {repconv}\n'
            line2 = f'Optimal dSCC: {repspear}\n'
            line3 = f'Final MSE loss: {repmse}\n'
            f.writelines([line1, line2, line3])

        torch.save(repnet.state_dict(), f"{out_path}/" + name_trained + '_weights.pt')

        utils.WritePDB(repmod*100, f"{out_path}/" + name_trained + '_structure.pdb')

        print('Saved trained model corresponding to ' + filepath_trained + ' to ' + f"{out_path}/" + name_trained + '_weights.pt')
        print('Saved optimal structure corresponding to ' + filepath_trained + ' to ' + f"{out_path}/" + name_trained + '_structure.pdb')
        conversions = [repconv]
    
    model = Net()
    model.load_state_dict(torch.load(f"{out_path}/" + name_trained + '_weights.pt'))
    model.eval()
    ...
    utils.WritePDB(repmod*100, f"{out_path}/" + name_untrained + '_generalized_structure.pdb')
    print('Saved optimal, generalized structure corresponding to ' + filepath_untrained + ' to ' + f"{out_path}/" + name_trained + '_structure.pdb')

    with open(f"{out_path}/" + name_untrained + '_generalized_log.txt', 'w') as f:
            line1 = f'Optimal conversion factor: {repconv}\n'
            line2 = f'Optimal dSCC: {repspear}\n'
