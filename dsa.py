import argparse
from downstream_analysis import run_compartment, run_embedtad, run_mustache, run_flamingo
...
def main(args):
    ...
    if args.ab_comp:
        run_compartment.run_ab_compartment(
            args.input, args.res, args.start, args.end, args.output, args.chrom)
    if args.loop:
        run_mustache.run_mustache(args.input, args.output, args.res, args.chrom)
    if args.tads:
        run_embedtad.run_embedtad(args.input, args.output, args.res)
    if args.structure:
        run_flamingo.run_flamingo(
            args.input, args.output, args.res, args.chrom, args.start, args.end)
