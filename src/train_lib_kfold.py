...
from src.data_loader import TripletDatasetKF
...
class Trainer:
    ...
    def __init__(self, cfg, log, dataset_dict: List, load_snapshot: bool = False, isDistributed: bool = False) -> None:
        ...
        self.train_dl = None
        self.train_steps = len(self.dataset_dict)
        self.batch_size = self.cfg.data.batch_size
        self.val_dl = None
        self.val_steps = len(self.dataset_dict)
        ...
        total_iterations = max(1, len(self.dataset_dict) *
                               self.cfg.training.epochs)
        warmup_iterations = max(
            0, len(self.dataset_dict) * self.cfg.training.warmup_epochs)
    ...
    def train(self, max_epochs: int):
        ...
        try:
            ...
            for fold, (train_idx, val_idx) in enumerate(kfold.split(self.dataset_dict)):
                print(f"--- FOLD {fold} ---")
                self.log.info(f"--- FOLD {fold} ---")
                train_set = [self.dataset_dict[i] for i in train_idx]
                train_ds = TripletDatasetKF(
                    triplet_dicts=train_set)

                val_set = [self.dataset_dict[i] for i in val_idx]
                val_ds = TripletDatasetKF(triplet_dicts=val_set)

                self.train_dl = get_dataloader(
                    ds=train_ds, batch_size=self.batch_size, shuffle=True, isDistributed=self.isDistributed)
                self.val_dl = get_dataloader(ds=val_ds, batch_size=self.batch_size,
                                             shuffle=False, isDistributed=self.isDistributed)
