...
class DiscoRandomWalks_binarizedMatrices:
    ...
    def compute_reproducibility(self, m1_csr, m2_csr, args, q=0.99):
        ...
        for t in range(args.tmin, args.tmax+1):
            ...
            plt.show()
            fname = args.outdir+'/'+args.outpref+'.'+args.m1name+'.vs.' + \
                args.m2name+'.DiscoRandomWalks_binarizedMatrices.' + \
                str(t)+'.png'
            plt.savefig(fname)
            reproducibility_text = reproducibility_text+'<img src="' + \
                os.path.basename(fname)+'" width="400" height="400"></td>'+'\n'
        reproducibility_text = reproducibility_text+'</tr>'+'\n'
