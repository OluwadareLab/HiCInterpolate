...
from src.metric.metrics import (
    get_lpips_from_tensor,
    get_psnr_from_tensor,
    get_ssim_from_tensor,
)
...
OUTPUT_DIR = f'/home/hc0783@unt.ad.unt.edu/workspace/hicinterpolate/datasets/final_output/test_dir/{MODEL_SUBDIR}'
...
def collate_fn(batch):
    batch = [
        b for b in batch
        if b is not None and all(element is not None for element in b)
    ]
...
def eval_all_metrics(pred: torch.Tensor, target: torch.Tensor, resol, h, lbr=0, ubr=40000) -> Dict[str, float]:
    return {
        "psnr": scalar(get_psnr_from_tensor(pred, target)),
        "ssim": scalar(get_ssim_from_tensor(pred, target)),
        "genome_disco": scalar(compute_genomedisco.compute_reproducibility_from_tensor(pred, target)),
        "hicrep": scalar(compute_hicrep.get_hicrep_scc_from_tensor(pred, target, resol, h, lbr, ubr)),
        "lpips": scalar(get_lpips_from_tensor(pred, target)),
    }
...
def process_chromosome(cfg, model, device, ds, batch_size: int, job_meta: dict, save_plots: bool) -> dict:
    ...
    try:
        with torch.no_grad():
            model.eval()
            is_drawn = False
            for _, (x0, y, x1) in enumerate(tqdm(dl)):
                x0 = x0.to(device)
                y = y.to(device)
                x1 = x1.to(device)
                pred_hic = model(x0, x1)

                if torch.isnan(pred_hic).any().item() or torch.isinf(pred_hic).any().item():
                    print(
                        f"[WARN] Invalid values detected in HiCInterpolate predictions for {job_meta['chromosome']} at batch starting index {seen}. Skipping this batch.")
                    continue
                pred_linear = linear_interpolation(x0, x1)
                pred_of = of_interpolation(x0, x1)
                out_path = heatmap_path(job_meta, 0)
                if not is_drawn:
                    plot.draw_hic_map(num_examples=min(NUM_VIZ_SAMPLES, len(
                        y)), x0=x0[:NUM_VIZ_SAMPLES], y=pred_hic[:NUM_VIZ_SAMPLES], pred=pred_hic[:NUM_VIZ_SAMPLES], x1=x1[:NUM_VIZ_SAMPLES], file=out_path)
                    is_drawn = True
                ...
                for method, pred in preds.items():
                    batch_metrics = eval_all_metrics(
                        pred, y, resol=job_meta["resolution"], h=1, lbr=0, ubr=(job_meta["patch_size"]-1) * job_meta["resolution"])
                ...
                del x0, y, x1, pred_hic, pred_linear, pred_of
    finally:
