...
OUTPUT_DIR="/home/hc0783.unt.ad.unt.edu/workspace/hicinterpolate/datasets/timeseries/full_triplets/output/hiccups"
...
mkdir -p "$OUTPUT_DIR"
echo "[run_hiccups] output root: $OUTPUT_DIR"

for resolution in "${RESOLUTIONS[@]}"; do
    res_tag="$(get_res_tag "$resolution")"
    echo "[run_hiccups] resolution: $resolution ($res_tag)"
    for model_patch in "${PATCHES[@]}"; do
        echo "[run_hiccups] model patch: $model_patch"
        for entry in "${HUMAN_TRIPLETS[@]}"; do
            ...
            for chromosome in "${HUMAN_CHROMS[@]}"; do
                for algo in y pred of linear 4dmax; do
                    ...
                    mkdir -p "$sub_out_dir"
                    echo "[run_hiccups] human ${sample}/${subsample} chr${chromosome}: $matrix_file"

                    python3 "$SCRIPT_DIR/run_hiccups.py" \
        ...
        for entry in "${MOUSE_TRIPLETS[@]}"; do
            ...
            for chromosome in "${MOUSE_CHROMS[@]}"; do
                for algo in y pred of linear 4dmax; do
                    ...
                    mkdir -p "$sub_out_dir"
                    echo "[run_hiccups] mouse ${sample}/${subsample} chr${chromosome}: $matrix_file"
