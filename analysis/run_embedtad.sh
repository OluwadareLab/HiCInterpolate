...
OUTPUT_DIR="/home/hc0783.unt.ad.unt.edu/workspace/hicinterpolate/datasets/timeseries/full_triplets/output/embedtad"
...
mkdir -p "$OUTPUT_DIR"
echo "[run_embedtad] output root: $OUTPUT_DIR"

cd "$REPO_ROOT"

for resolution in "${RESOLUTIONS[@]}"; do
    res_tag="$(get_res_tag "$resolution")"
    echo "[run_embedtad] resolution: $resolution ($res_tag)"
    for model_patch in "${PATCHES[@]}"; do
        echo "[run_embedtad] model patch: $model_patch"
        for entry in "${HUMAN_TRIPLETS[@]}"; do
            ...
            for chromosome in "${HUMAN_CHROMS[@]}"; do
                for algo in y pred of linear 4dmax; do
                    ...
                    mkdir -p "$sub_out_dir"
                    echo "[run_embedtad] human ${sample}/${subsample} chr${chromosome}: $matrix_file"

                    if [[ ! -f "$matrix_file" ]]; then
                        echo "[run_embedtad] SKIP missing: $matrix_file" >&2
                        continue
                    fi

                    python3 "$SCRIPT_DIR/run_embedtad.py" \
        ...
        for entry in "${MOUSE_TRIPLETS[@]}"; do
            ...
            for chromosome in "${MOUSE_CHROMS[@]}"; do
                for algo in y pred of linear 4dmax; do
                    ...
                    mkdir -p "$sub_out_dir"
                    echo "[run_embedtad] mouse ${sample}/${subsample} chr${chromosome}: $matrix_file"
